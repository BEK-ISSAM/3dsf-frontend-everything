...
const AddCourse = () => {
  ...
  const [globalQuizId, setGlobalQuizId] = useState(null);
  ...
  const handleCourseSubmit = async (event) => {
    ...
    const courseData = {
      ...
      teacher: { id: JSON.parse(localStorage.getItem("user")).id },
    };
  ...
  const handleVideoSubmit = async (event) => {
    ...
    for (const video of videoFiles) {
      ...
      try {
        const url = `http://localhost:8080/api/videos?courseId=${courseId}&title=${encodeURIComponent(title)}`;
  ...
  const handleAssignmentSubmit = async (event) => {
    ...
    for (const assignment of assignments) {
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/assignments', assignmentData);
  ...
  const handleFaqSubmit = async (event) => {
    ...
    for (const faq of faqs) {
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/faqs', faqData);
  ...
  const handleQuizSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8080/api/quizzes/course/${courseId}`, quizData, {
        headers: { 'Content-Type': 'application/json' },
      });
      const quizId = response.data.id;
      setGlobalQuizId(quizId);
      console.log(globalQuizId)

      if (quizId) {
        setSuccessMessage('Quiz created successfully!');
        setQcmFormVisible(true); // Show QCM form after creating quiz
      } else {
  ...
  const handleQcmSubmit = async (event) => {
    ...
    if (!courseId || !globalQuizId) {
      console.error('No course ID or quiz ID available');
      return;
    }

    for (const qcm of qcms) {
      const { question, correctAnswer, image } = qcm;
      ...
      formData.append('qcm', JSON.stringify(qcmData));
      ...
      try {
        const response = await axios.post('http://localhost:8080/api/qcm', formData, {
  ...
  const handleQcmChange = (index, field, subIndex, value) => {
    ...
    if (field === 'possibleAnswers') {
      ...
      newQcms[index][field] = value.target.files[0];
    } else {
  ...
  const addQcmField = () => {
    setQcms([...qcms, { question: '', correctAnswer: '', possibleAnswers: [''], image: null }]);
  };
        ...
        <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-md rounded-lg">
          ...
          {!videoUploadFormVisible && (
            <form onSubmit={handleCourseSubmit} className="space-y-6">
              ...
              {/* New field for thumbnail image */}
              <div className="flex flex-col">
                <label className="text-gray-700 mb-2" htmlFor="thumbnail">Thumbnail Image</label>
                <input
                  type="file"
                  id="thumbnail"
          ...
          {/* Quiz Form */}
          <h3 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Create Quiz</h3>
          <form onSubmit={handleQuizSubmit}>
            <div className="flex flex-col mb-4">
              <label className="text-gray-700 mb-2">Quiz Title</label>
              <input
                type="text"
                value={quizTitle}
          ...
          {qcmFormVisible && (
            <form onSubmit={handleQcmSubmit}>
              <h3 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Add Quiz Questions</h3>
              {qcms.map((qcm, index) => (
                <div key={index} className="mb-6">
                  <div className="flex flex-col mb-4">
                  ...
                  <div className="flex flex-col mb-4">
                    <label className="text-gray-700 mb-2">Upload Image (Optional)</label>
                    <input
                      type="file"
                      onChange={(e) => handleQcmChange(index, 'image', null, e)}
                      className="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </div>
                </div>
